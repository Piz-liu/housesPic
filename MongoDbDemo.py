...
class GFS:
    ...
    def _connect():
        if not GFS.c:
            GFS.c = Connection("mongodb://127.0.0.1:27017")  # 建立mongodb的连接
            GFS.db = GFS.c['hw-sq']  # 连接到指定的数据库中
            GFS.fs = GridFS(GFS.db, collection='fs')  # 连接到具体的collection中
...
if __name__ == '__main__':
    image_name = u'F:\gongxiang\lptp\济南楼盘\houses_type_pic\\2-221-90-A.jpg'
    if image_name:
        gfs = GFS.getInstance()
        if gfs:
